...
from core import buttons, screens
...
class TemplateEditorTests(unittest.TestCase):
    ...
    def save(self,**changes):
        payload={'token':'token','region':[17,23,51,60],'kind':'buttons','name':'battle','variant':'test'}
    ...
    def test_saved_crop_preserves_exact_pixels_and_is_discovered(self):
        ...
        self.assertEqual(buttons.template_files('battle'),[self.root/result['path']])
    ...
    def test_invalid_names_bounds_and_expired_capture(self):
        for change in [{'variant':'../escape'},{'name':'../battle'},{'region':[-1,0,20,20]},{'region':[0,0,121,30]},{'region':[0,0,1,1]},{'region':[True,0,20,20]},{'token':'missing'}]:
    ...
    def test_screen_crop_is_compared_at_original_position(self):
        result=self.save(kind='screens',name='home')
        with patch.object(screens,'_reference_files',side_effect=lambda state:[self.root/result['path']] if state==screens.Screen.HOME else []):
            match=screens.identify_screen_details(self.frame,threshold=.99)
            self.assertEqual(match.screen,screens.Screen.HOME)
            self.assertEqual(match.score,1)
            wrong_size=np.zeros((100,120,3),np.uint8)
            self.assertEqual(screens.identify_screen(wrong_size),screens.Screen.UNKNOWN)
