...
Use **Capture for template**, drag a rectangle on the lossless screenshot, and adjust Left/Top/Right/Bottom for pixel-precise bounds. The crop preview and dimensions update immediately. Choose Button or Screen area, a logical target, and a unique variant name; **Save template** saves a lossless PNG from the original server-side frame. Download saved PNG exports that same crop.
