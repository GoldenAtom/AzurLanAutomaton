...
For low-latency control from the Windows workstation, run `Start-scrcpy-local.cmd` in the project folder. It discovers Waydroid's current address over SSH, opens a private local SSH tunnel, and starts the official native Windows scrcpy client at native 1920×1080, up to 30 FPS and a requested 16 Mbps. Automatic downscaling is disabled and delivered FPS is printed in the console. Closing scrcpy also closes its tunnel. This is the preferred interactive path because the video is decoded only once.
...
`scripts/install-scrcpy-windows.ps1` installs the checksum-verified official Windows x86-64 release into ignored `local-tools/`. `scripts/install-scrcpy-user.sh` does the same for Debian under `~/.local/share/azurlane/scrcpy`. The on-demand `azurlane-scrcpy.service` runs the browser fallback after the existing `waydroid-headless.service`; it is disabled at boot so it does not compete with the direct stream. Override the Debian binary with `AUTOMATON_SCRCPY_PATH` or the device with `AUTOMATON_ADB_DEVICE` in the environment file.
...
Use **Capture for template**, drag a rectangle on the lossless screenshot, and adjust Left/Top/Right/Bottom for pixel-precise bounds. The crop preview and dimensions update immediately. Choose or create a target folder, choose Button, Screen area, or Number/OCR, then enter a new template name. The variant identifies an alternate image of that same template; the first image defaults to `custom`. If the folder has no templates and you entered a name in Variant instead, Save uses that name for the new template with a `custom` variant, and displays the resolved path before saving. For example, the `battle` target can contain a `campaign_selector` screen and a `start` button. **Save template** saves a lossless PNG from the original server-side frame. Download saved PNG exports that same crop.
...
An **End with error** block produces a concise authored-program warning and a failed run status, without an internal Python traceback. Every Press reports its template score, threshold, and whether it tapped in the run events. A failed Press also saves its pre-tap frame under ignored `debug/press_miss_*` files on the appliance for template diagnosis.

**Run saved** starts an explicit run in `azurlane-bot.service`. Editing/saving programs does not change that run: the called program library is snapshotted at start. Closing the browser does not stop a run. **Stop program** cancels at the next check; an in-flight ADB command can take its configured timeout. Stopping/restarting the bot service also ends the run. Crashed/interrupted runs are never automatically resumed or replayed. The overall run limit and each While loop's limit can be `0` for unlimited execution, or set to a finite safety limit up to 24 hours. Calls cannot recurse.

The visual language is computationally universal (Turing complete in the programming-language sense): it has arbitrary-size integer counters, arithmetic, conditional branches, and unlimited While loops. As with every real implementation, execution is still constrained by the Debian machine's finite memory and uptime.
