(() => {
 ...
 function templateChoice(){const name=el('template-new-name').value.trim()||el('template-name').value,variant=el('template-variant').value.trim();return name?{name,variant}:{name:variant==='custom'?'':variant,variant:'custom'};}
 function destination(){const {name,variant}=templateChoice(),slug=/^[a-z0-9][a-z0-9_-]{0,47}$/;el('template-destination').textContent=!name?'Enter a new template name. An entered alternate-image name can also create the first template.':![target(),name,variant].every(value=>slug.test(value))?'Folder, template and variant names must use 1–48 lowercase letters, digits, hyphens or underscores.':`Save as: local-templates/targets/${target()}/${el('template-kind').value}/${name}/${variant}.png${name===el('template-variant').value.trim()&&!el('template-new-name').value.trim()&&!el('template-name').value?' (new template; first variant is custom)':''}`;}
 function names(){el('template-name').replaceChildren();const kind=el('template-kind').value,selected=target();for(const asset of frame.assets[kind].filter(item=>item.target===selected)){const option=document.createElement('option');option.value=asset.name;option.textContent=asset.name;el('template-name').append(option);}if(!el('template-name').options.length){const option=document.createElement('option');option.value='';option.textContent='Create new template';el('template-name').append(option);}destination();}
 ...
 el('template-save').onclick=async()=>{
  ...
  try{const folder=target(),{name,variant}=templateChoice(),kind=el('template-kind').value;const result=await request('save',{token:frame.token,region:selection,kind,target:folder,name,variant});status(`${result.message} Saved ${result.path} (${result.width} × ${result.height}).`);el('template-download').href=result.image;el('template-download').download=result.filename;el('template-download').hidden=false;
   ...
   el('template-new-target').value='';el('template-new-name').value='';el('template-variant').value='custom';targets();el('template-target').value=folder;names();el('template-name').value=name;destination();
